...
func (k msgServer) CancelDeal(goCtx context.Context, msg *types.MsgCancelDeal) (*types.MsgCancelDealResponse, error) {
	...
	if deal.Status == types.Deal_SCHEDULED || deal.Status == types.Deal_INITIALIZED {
		...
		k.bankKeeper.SendCoinsFromModuleToAccount(ctx, types.ModuleName, sdk.AccAddress(deal.Requester), sdk.NewCoins(sdk.NewInt64Coin(manduTypes.TokenDenom, int64(deal.AvailableAmount))))
	...
	if ctx.BlockHeight() < int64(deal.StartBlock) {
		if msg.Amount != 0 {
			if msg.Amount < deal.TotalAmount {
				...
				k.bankKeeper.SendCoinsFromModuleToAccount(ctx, types.ModuleName, requester, sdk.NewCoins(sdk.NewInt64Coin(manduTypes.TokenDenom, int64(amountToReturn))))
